...
%%%%%%% PART B
b) Let $x_+$ denote positively label $y = +1$, $X_+$ the set containing them and $ \mu_{I_+} =  \frac{1}{n_+}\sum_{x_+ \in X_+} \Phi(x) $. Similarly, define $x_-$, $X_-$ and $ \mu_{I_-}$.
...
since $d(x, \mu_-)$ is only in terms of kernel products as expressed above, this is the classifier.
